...
class NameClassifierBuilder (ProcessedDataBuilderBase):
    ...
    def _buildProcessedData(self) -> None:
        ...
        for name in self._preprocessedData:
            ...
            self._processedData.append(self.__currentNameMapping)
    ...
    def __initializeNameMapping(self) -> None:
        ...
        self.__currentNameMapping = []
        for i in range(6):
            self.__currentNameMapping.append(0.0)
    
    def __mapTitle(self, title) -> None:
        """Maps a title to a bin"""
        if(title in ['Don', 'Major', 'Capt', 'Jonkheer', 'Rev', 'Col', "Mr"]):
            self.__currentNameMapping[0] = 1.0
        elif(title in ['Countess', 'Mme', 'Mlle', 'Ms', 'Miss']):
            self.__currentNameMapping[1] = 1.0
        elif(title in ['Mrs']):
            self.__currentNameMapping[2] = 1.0
        elif(title in ['Dr']):
            self.__currentNameMapping[3] = 1.0
        elif(title in ['Master']):
            self.__currentNameMapping[4] = 1.0
        else:
            self.__currentNameMapping[5] = 1.0